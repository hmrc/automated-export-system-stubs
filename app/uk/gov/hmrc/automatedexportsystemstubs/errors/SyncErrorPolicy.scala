...
object SyncErrorPolicy:
  ...
  def syncErrorFor(xml: Elem): Option[SyncError] =
    mrn(xml).flatMap {
      ...
      case v if v.endsWith("A2") => Some(SyncError(500, "INTERNAL_SERVER_ERROR", "Server error"))
      case v if v.endsWith("A3") => Some(SyncError(400, "VALIDATION_ERROR", "Validation error"))
      case _                     => None
    }
